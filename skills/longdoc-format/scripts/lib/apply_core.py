...
Used by the portable skill (``scripts/lib/apply_core.py``). The workflow tree
keeps a sibling copy that can fall back to AutoDataBuild's officecli runner;
this skill copy only talks to ``lib.officecli``.
...
from lib.cell_plan import coerce_slot, designed_slots, resolve_cell_slot
...
_HEADER_FOOTER_KEYS = {
    ...
    "type", "direction", "font", "font.ea", "font.latin",
}
_CELL_KEYS = {
    "valign", "fill", "shading",
...
def _load_backend() -> dict[str, Any]:
    ...
    raise RuntimeError(
        "apply_core.py must sit next to officecli.py in scripts/lib/ "
        "(standalone longdoc-format skill layout)."
    )
...
def _hf_writable(chrome: dict[str, Any] | None, *, first: bool = False) -> dict[str, Any]:
    ...
    for k, v in (chrome or {}).items():
        ...
        out[k] = v
    if "font" not in out:
        fea = out.get("font.ea") or out.get("font.latin")
        if fea:
            out["font"] = fea
    if first:
        out["type"] = "first"
...
def _section_hf_from_spec(spec: dict[str, Any]) -> tuple[dict | None, dict | None, dict | None]:
    """Read header/footer from top-level A keys or nested _header_footer."""
    def _as_dict(value: Any) -> dict | None:
        return value if isinstance(value, dict) and value else None

    header = _as_dict(spec.get("header"))
    footer = _as_dict(spec.get("footer"))
    header_first = _as_dict(spec.get("header_first"))
    nested = spec.get("_header_footer")
    if not isinstance(nested, dict):
        props = spec.get("props") if isinstance(spec.get("props"), dict) else {}
        nested = props.get("_header_footer") if isinstance(props, dict) else None
    if isinstance(nested, dict):
        header = header or _as_dict(nested.get("header"))
        footer = footer or _as_dict(nested.get("footer"))
        header_first = header_first or _as_dict(nested.get("header_first"))
    return header, footer, header_first
    ...
    for loc_id, sid in mapping.items():
        ...
        if bag:
            ...
            if bag.get("pgBorders") not in (None, "", "none"):
                ...
                if cmd:
                    commands.append(cmd)
        header, footer, header_first = _section_hf_from_spec(spec)
...
def _compile_tables(
    ...
    for loc_id, sid in mapping.items():
        ...
        n_rows, n_cols = _grid_size(inv_cells)
        ...
        if cell_map:
            ...
            for item in cell_map:
                ...
                if para_sid:
                    ...
                    para_path = str(hit.get("para_path") or "") or (path + "/p[1]" if "/p[" not in path else path)
                    para_alts = [a + "/p[1]" for a in alts if "/p[" not in a]
...
def _grid_size(inv_cells: dict[tuple[int, int], dict]) -> tuple[int, int]:
    if not inv_cells:
        return 0, 0
    return max(r for r, _c in inv_cells), max(c for _r, c in inv_cells)
