"""officecli helpers for LongDocFormatter (resident open + query/get/set)."""
...
from LongDocFormatter.officecli.read._ooxml_outline import body_outline_index, coerce_outline_lvl
from LongDocFormatter.officecli.read._ooxml_section import (
    section_index_for_picture_path,
    section_index_for_table_path,
    section_preview_text,
)
from LongDocFormatter.workflow.contracts import DocElement, Layer
...
def _neighbor_window(para_seq: List[DocElement], idx: int, n: int) -> tuple[list, list]:
    """Up to *n* nonempty paragraphs before/after a block at *idx* in para_seq."""
    if n <= 0:
        return [], []
...
def attach_neighbor_paragraphs(
    ...
    *,
    ...
    """Store up to *max_n* nonempty body paragraphs before/after each table and image."""
...
def inventory_bundle(doc: Path, *, profile: str = "full") -> Dict[str, Any]:
    ...
    Neighbors (nonempty paragraphs) are attached on ``full`` and ``assign`` so
    table/image assignment can see captions. Section leading-text preview is
    still filled on assign.
    """
